...
// This backend runs on localhost 5000 while the frontend runs on a dynamically allocated port on localhost.
// This means that we have to enable cors otherwise we will get a cors error.
// So I installed the cors package to enable cors.
var cors = require('cors');

// As recommended, I used express for the backend. 
const express = require("express");
const app = express();

// I am using a mysql database server.
// So I installed the mysql2 package so that I could read from and write to the database.
...
// This API entry point sends the requester an image associated with the id provided.
app.get('/images/:id',(req,res)=>
{
    // Parsing the request to prevent folder traversal and then sending the file.
    res.sendFile('/images/'+parseInt(req.params.id)+'.jpg');
});

// Connecting this server to the database
const connection = mysql.createConnection({
    host: 'localhost',
    user: 'user',
    password: 'A super long password that will never get hacked.',
    database: 'khula_assessment'
});

// Defining a reusable const function for making queries
const makeQuery = function(query,response)
                  {
...
// This entry point sends at most two objects per request. 
// Designed for the lazy loading bonus marks. 
app.get('/:id',(req,res)=>
{
    // Parsing the request to prevent SQL injection attacks and enable multiplication. 
    // Then multiplying the request id by 2 so that we only search through 2 unique IDs at a time.
    const Unique_SQL_injection_proof_ID = parseInt(req.params.id) * 2;
    makeQuery('SELECT * FROM School WHERE Id = '+(Unique_SQL_injection_proof_ID - 1)+' OR Id ='+Unique_SQL_injection_proof_ID+';',res);
});

// This entry point was made to auto populate the database and make life easier.
// Note that it auto populates from the 85th row onwards... because I had already manually inserted 84 rows.
app.get('/post/data',(req,res)=>
{
    // The dataOfLocations variable is from github and contains the 1000 largest cities in the US from 2018. 
    const dataOfLocations = [
        {
            "city": "New York", 
    ...
    // This arrayOfNames contains the nick names I created for respective schools. 
    const arrayOfNames = ["Winst-Sal","Glen","Garl","Hial","Reno","Chesa","Gilb","Baton","Irv","Scott","NLV","Frem","Bois","Rich","San Bern","Birm","Spok","Roch","Des Moi","Modes","Fayet","Taco","Oxn","Font","Colu","Montgom","Moreno","Shrev","Aur","Yonk","Akr","Hunt Beach","lil roc","Aug-Ric","Amar","Glen","Mobi","Gran Rap","SLC","Tallah","Hunstv","Gran Prai","Knox","Worce","NewPo New","Browns","Over Park","Sant Clar","Provi","Gard Grov","Chatt","Oceans","Jack","Fort Laud","Sant Ros","Ranch Cuc","Port St. Lu","Tempe","Ont","Vancouv","Cape Cor","Siou Fall","theSimpson","Peor","Pemb","Elk Grov","Salem","Lanc","Covivi","Eug","Palm","Sali","Spring","Pasa","Fort Coll","Haywa","Pomo","Cary","Rockf","Alex","Esco","Mckin","Kans","Jolie","Sunyval","Torr","Bridge","Lake","Holly","Pater","Naper","Syra","Mesq","Dayt","Sava","Clark","Oran","Pasa","Fuller","Kill","Fris","Hampt","McAll","Warr","Belle","Wes Val City","Colu","Ola","Ster","New Hav","Mira","Waco","Thou Oak","Ced Rap","Charles","Visa","Topek","Eliz","Gains","Thorn","Rose","Carrol","Coral Spr","Stam","Simi Val","Conc","Hart","Kent","Lafa","Mid","Surp","Dent","Vict","Evan","Sant Clar","Abil","Ath-Clar","Vall","Allen","Norm","Beaum","Indpen","Murf","Ann Arb","field","Berk","Peor","Provo","El Mon","Bia","Lans","Farg","Down","Cost Mes","Wilm","Arva","Ingle","Miam Be","Carls","Westmin","Roches","Ode","Manch","Elgin","Wes Jord","Rou Roc","Clear","Water","Gresh","Fair","Bill","Low","San Buen","Pueb","Hi Point","Wes Cov","Rich","Murr","Camb","Anti","Teme","Norw","Cent","Ever","Palm Bay","Wich Fall","Green Bay","Dally","Burb","Richard","Pomp Beach","North Charl","Broke Arro","Bould","Wes Palm","Sant Mar","El Caj","Daven","Rial","Las Cru","San Mat","Lewis","Sou Ben","Lake","Erie","Tyl","Pear","Coll","Kenos","Sandy","Clovis","Flint","Roan","Alb","Juru","Compt"];
    for(let i = 85; i < 301; i++)
    {
        makeQuery("INSERT INTO School (Id,Name,Address,Latitude,Longitude,ImageLink) VALUES ("+i+",'"+arrayOfNames[i-85]+" School"+"','"+dataOfLocations[i].city+"','"+dataOfLocations[i].latitude+"','"+dataOfLocations[i].longitude+"','http:localhost:5000/images/"+i+"');",null);
    }
    res.write("auto population complete");
    res.end();
});

// This entry point scans through the whole table and returns the data set that match the searched name.
app.get('/school/name/:name',(req,res)=>
{
    // The use of 'LIKE' gives implicit protection of from sql injections in this specific case.
    // This is because the search has to be decodable. Otherwise the error "URIError: Failed to decode param /school/name/..." will pop up.
    makeQuery("SELECT * FROM School WHERE Name LIKE '%"+req.params.name+"%';",res);
});
